...
def get_dob(args: argparse.Namespace) -> dt.date:
    """Read DOB from flag or interactive prompt."""
    if args.dob:
        return parse_dob(args.dob)

    dob_input = input("Enter your date of birth (YYYY-MM-DD): ").strip()
    if not dob_input:
        raise ValueError("DOB is required. Please enter a value like 1998-12-31.")
    return parse_dob(dob_input)
